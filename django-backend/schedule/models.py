...
class Reservation(models.Model):
        ...
        def clean(self):
            ...
            reservations = Reservation.objects.filter(owner = self.owner)
            for r in reservations:
                if (r.slot.start < self.slot.start < r.slot.end or r.slot.start < self.slot.end < r.slot.end):
                    raise APIException(detail='Reservations can overlap an existing reservation',code='400')
				#This is needed because time slots can be created with a sentinel end time, equal to the start time.
                elif (self.slot.start < r.slot.start < self.slot.end or self.slot.start < r.slot.end < self.slot.end):
                    raise APIException(detail='Reservations can overlap an existing reservation',code='400')
